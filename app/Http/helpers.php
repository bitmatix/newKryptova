...
function checkSelectedCurrencyInGBP($currency, $amount, $selected = '') {
    ...
    $data = file_get_contents('https://apilayer.net/api/live?access_key=da76f4a49243dd37ea7532037f2dde01&currencies=' . $currency . '&source=' . $selected . '&format=1');
    $dd = json_decode($data);
...
function getAllCurrenciesRates(){
    $data = file_get_contents('https://apilayer.net/api/live?access_key=da76f4a49243dd37ea7532037f2dde01&currencies=&source=USD&format=1');
    $allRates = json_decode($data);
    return $allRates->quotes;
}

function getConversionAmount($allRates,$currency,$amount=0){
    $conversionAmount=0;
    if($allRates){
        $selector = "USD".$currency;
        $conversionAmount=(float) round(($amount / $allRates->$selector), 2);
    }
...
function convertMonthlyFee($currency, $amount) {
    ...
    $data = file_get_contents('https://apilayer.net/api/live?access_key=da76f4a49243dd37ea7532037f2dde01&currencies=' . $currency . '&source=' . $default_currency . '&format=1');
    $dd = json_decode($data);
...
function currency_convert_into_usd($currency, $amount) {
     ...
     */
    $apiCC = 'https://apilayer.net/api/live?access_key=da76f4a49243dd37ea7532037f2dde01&currencies=USD&source='.$currency.'&format=1';
...
function postCurlRequestBackUpOne(string $url, array $params) {
    ...
    $parts=parse_url($url);
    // $parts=parse_url('https://system-unixbroker.com/b7cs/IPayTotal/Callback');
    // dd($parts);
...
function postCurlRequest(string $url, array $post_array, $check_ssl=true) {
    // $url = 'https://process.realdeposits.com/api/gateway/notification/IPayTotal/1338481';
