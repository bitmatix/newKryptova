...
class UserManagementController extends AdminController
{
    ...
    */ // ==============================================
        ...
        return \DataTables::of($data)
            ...
            ->addColumn('Actions', function ($data) {
                if (auth()->guard('admin')->user()->can(['user-management-action'])) {
                    return '<a href=' . \URL::route('assign-mid', $data->id) . ' class="btn btn-info btn-outline">Assign MID</a>
                        ...
                        <a data-toggle="modal" href="#changePass" class="btn btn-danger btn-outline changePassBtn" data-id="' . $data->id . '">Change Password</a>';
                } else {
                    return '---';
                }
            })
            ...
            ->addColumn('delete', function ($data) {
                if (auth()->guard('admin')->user()->can(['user-management-delete'])) {
                    return '<a class="btn btn-circle btn-icon-only btn-danger remove-record" data-target="#custom-width-modal" data-toggle="modal" data-url="' . \URL::route('user-management-delete', $data->id) . '" data-id="' . $data->id . '">
            ...
            ->addColumn('ID', function ($data) {
                return '<a data-toggle="modal" href="#showTransactionAmount" class="showTransactionAmount text-info" data-id="' . $data->id . '"><strong>' . $data->id . '</strong></a>';
            })
    ...
    public function userTotalAmount(Request $request, $id)
    {
        ...
        if ($count == 0) {
            notificationMsg('warning', 'No any transactions found in system.');
    ...
    {
        $midData = \DB::table('middetails')->where('mid_type', '1')->whereNull('deleted_at')->get();
        $cryptoMIDData = \DB::table('middetails')->where('mid_type', '3')->whereNull('deleted_at')->get();
        $bankMIDData = \DB::table('middetails')->where('mid_type', '2')->whereNull('deleted_at')->get();
        ...
        return view($this->moduleTitleP . '.assign_MID', compact('midData', 'data', 'cryptoMIDData', 'bankMIDData', 'midListInArray','midListInArrayVisa','midListInArray','arrMainVisa','midListInArrayAll','arrMainMaster'));
    ...
    public function assignMIDStore(Request $request)
    {
        ...
        // if($input['mid'] != '1' && $input['mid'] != '0'){

        //     $input['live_mid'] = $input['mid'];

        //     if(isset($input['card_visa_mid'])){
        //         $input['live_card_visa_mid'] = $input['card_visa_mid'];
        //     }
        //     if(isset($input['card_master_mid'])){
        //         $input['live_card_master_mid'] = $input['card_master_mid'];
        //     }
        // }
        ...
        if (isset($input['multiple_mid'])) {
            //echo array_search($input['mid'], $input['multiple_mid']);//exit();
            if (($key = array_search($input['mid'], $input['multiple_mid'])) !== false) {
                unset($input['multiple_mid'][$key]);
            }
            $input['multiple_mid'] = json_encode(array_values($input["multiple_mid"]));
        }else{
            $input['multiple_mid'] = "";
        }
    ...
    */ // ==============================================
    ...
    public function setAgent(Request $request)
    {
        ...
        if ($validator->passes()) {
            if ($this->user->updateData($request->user_id, ['agent_id' => $request->agent_id, 'agent_commission' => $request->commission,"agent_commission_master_card"=>$request->commission_master])) {
                return response()->json([
                    'success' => true,
                ]);
            } else {
    ...
    */ // ==============================================
    ...
    public function merchantRules($id)
    {
        ...
        $data = User::select('users.*')->where('users.id', $id)->first();
        return view($this->moduleTitleP . '.merchantRules', compact('data', 'CardRules', 'CryptoRules', 'BankRules', 'id'));
    }
}
