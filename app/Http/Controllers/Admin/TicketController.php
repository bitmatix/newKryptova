...
class TicketController extends Controller
{
    ...
    {
        $tickets = $this->ticket->getAdminTickets();

        return view($this->moduleTitleP . '.index', compact('tickets'));
    }
        ...
        return \DataTables::of($data)
            ...
            ->addColumn('department', function ($data) {
                if ($data->department == '1') {
                    return '<span class="badge badge-info">Technical</span>';
            ...
            ->addColumn('Actions', function ($data) {
                $html = '<a href="' . \URL::route('admin.ticket.show', [$data->id]) . '" class="btn btn-sm btn-icon waves-effect waves-light btn-info"><i class="fa fa-eye"></i></a>
                    <a class="btn btn-icon text-white btn-sm btn-danger waves-effect waves-light remove-record" data-target="#custom-width-modal" data-toggle="modal" data-url="' . \URL::route('admin.ticket.destroy', $data->id) . '" data-id="' . $data->id . '"><i class="fa fa-trash"></i></a>';
                if ($data->status != '2') {
    ...
    public function show($id)
    {
        ...
        if ($ticket->user_id) { 
            ...
            return view($this->moduleTitleP . '.show', compact('ticket','user'));
        }
