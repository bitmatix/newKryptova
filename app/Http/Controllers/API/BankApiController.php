...
use App\Http\Controllers\Repo\BankTransactionRepo;
...
class BankApiController extends Controller
{
    ...
    */// ==============================================
    public function __construct()
    {
        ...
        $this->BankTransactionRepo = new BankTransactionRepo;
    }
    ...
    */// ==============================================
    public function store(Request $request)
    {
        ...
        // if the bank mid not assign.
        if(!$payment_gateway_id->bank_mid || $payment_gateway_id->bank_mid == null || $payment_gateway_id->bank_mid == '') {
            return response()->json([
                'status' => 'fail',
                'message' => 'Bank transaction API not unable on your account, Please contact in support.',
                'data' => [
                    'order_id' => null,
                    'amount' => isset($input['amount'])?$input['amount']:null,
                    'currency' => isset($input['currency'])?$input['currency']:null,
        ...
        // gateway object
        $check_assign_mid = checkAssignMID($payment_gateway_id->bank_mid);
        // dd($check_assign_mid);
        ...
        $request->merge([
            ...
            'payment_gateway_id' => $payment_gateway_id->bank_mid,
        ...
        $requestInput = \Arr::except($request->all(), array('request_from_type', 'token'));
        // send request to transaction repo class
        $return_data = $this->BankTransactionRepo->store($requestInput);
        // dd($return_data);
