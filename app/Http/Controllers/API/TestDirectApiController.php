...
class TestDirectApiController extends Controller
{
    ...
    */// ==============================================
    public function store(Request $request)
    {
        ...
        $payment_gateway_id = DB::select("select `middetails`.`id` as `midid`, `middetails`.`gateway_table`, `users`.* from `users` left join `middetails` on `middetails`.`id` = 1 where `users`.`api_key` = '" . $input['api_key'] . "' and `users`.`is_active` = '1' and `users`.`deleted_at` is null
");
        if (! empty($payment_gateway_id[0])) {
            $payment_gateway_id = $payment_gateway_id[0];
            $payment_gateway_id->mid = 1;
            $payment_gateway_id->amexmid = 1;
            $payment_gateway_id->visamid = 1;
            $payment_gateway_id->mastercardmid = 1;
            $payment_gateway_id->discovermid = 1;
        }
        ...
        $return_data = $this->test_transaction_repo->store($request->all());
        // dd($return_data);
