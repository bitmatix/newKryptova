    ...
    public function store(Request $request)
    {
        ...
        if(isset($input['request_from_type']) && isset($input['token'])) {
            ...
            if(!$payment_gateway_id->crypto_mid || $payment_gateway_id->crypto_mid == null || $payment_gateway_id->crypto_mid == '') {
                return response()->json([
                    'status' => 'fail',
                    'message' => 'Crypto transaction API not unable on your account, Please contact in support.',
                    'data' => [
                        'order_id' => null,
                        'amount' => isset($input['amount'])?$input['amount']:null,
                        'currency' => isset($input['currency'])?$input['currency']:null,
            ...
            // gateway object
            $check_assign_mid = checkAssignMID($payment_gateway_id->crypto_mid);          
        }        
        ...
        if(isset($input['request_from_type']) && isset($input['token'])) {
            ...
            $PaymentMID = $payment_gateway_id->crypto_mid;
        }
