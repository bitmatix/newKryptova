...
class ReportController extends Controller
{
    public function getReport(Request $request)
    {
        ...
        if (!empty($slave_connection)) {
            ...
            _WriteLogsInFile($getDatabaseName . " connection from RP report", 'slave_connection');
        }
        ...
        if ($request->start_date) {
            ...
            $TransactionSummary =   $TransactionSummary->where('trans.transaction_date', '>=', $start_date);
        }
        if ($request->end_date) {
            $end_date = date('Y-m-d', strtotime($request->end_date));
            $TransactionSummary =   $TransactionSummary->where('trans.transaction_date', '<=', $end_date);
        }
    ...
    public function cardSummaryReport(Request $request)
    {
        ...
        if (!empty($slave_connection)) {
            \DB::setDefaultConnection($slave_connection);
            $getDatabaseName = \DB::connection()->getDatabaseName();
            _WriteLogsInFile($getDatabaseName . " connection from RP card report", 'slave_connection');
        }
    ...
    public function paymentStatusReport(Request $request)
    {
        ...
        if (!empty($slave_connection)) {
            \DB::setDefaultConnection($slave_connection);
            $getDatabaseName = \DB::connection()->getDatabaseName();
            _WriteLogsInFile($getDatabaseName . " connection from RP payment status report", 'slave_connection');
        }
