...
class BTPay extends Controller
{
    use StoreTransaction, CCDetails;

    // const BASE_URL = 'https://co.bestoppay.com/TestTPInterface'; // test
    const BASE_URL = 'https://co.bestoppay.com/TPInterface'; // live
    ...
    public function checkout($input, $check_assign_mid) {
        ...
        \Log::info([
            'btpay-cardtype' => $cardType,
            'btpay-order_id' => $input['order_id']
        ]);
        ...
        return [
            ...
            'redirect_3ds_url' => route('btpay-confirmation', [
    ...
    public function confirmation(Request $request, $session_id, $cardDetails) {
        ...
        try {
            ...
            $signInfo = $input['merNo'].$input['gatewayNo'].$input['orderNo'].$input['currency'].$input['amount'].$input['first_name'].$input['last_name'].$cardDetails[0].$cardDetails[1].$cardDetails[2].$cardDetails[3].$input['email'].$input['signkey'];
            ...
            \Log::info([
                'btpay-payment-form-exception' => $e->getMessage()
            ]);
        }
        
        return view('gateway.btpay', compact('error', 'input', 'session_id', 'cardDetails'));
    ...
    public function getCredential($payment_gateway_id,$cardType){
        ...
        if($cardType == 3) { 
            ...
            return [
                ...
                $check_assign_mid->gatewayNo_master,
                $check_assign_mid->signkey_master
            ];
        }
       //american express card 
        if($cardType == 1) {
            ...
            return [
                $check_assign_mid->merNo,
                $check_assign_mid->gatewayNo_AE,
                $check_assign_mid->signkey_AE];
        }
        // JCB card
        if($cardType == 5) {
            ...
            return [
                $check_assign_mid->merNo,
                $check_assign_mid->gatewayNo_JCB,
                $check_assign_mid->signkey_JCB
            ];
        }  
    }
}