...
class Texcent extends Controller
{
    ...
    {
        // all manually blocked condition
        if(!in_array($input['amount'], ['10', '30', '50', '70', '170', '250', '500', '750'])) {
            return [
                'status' => '5',
                'reason' => 'Amount should be mathch with [10, 30, 50, 70, 170, 250, 500, 750]',
                'order_id' => $input['order_id'],
            ];
        }

        if(!in_array($input['currency'], ['USD','HKD','GBP','JPY','EUR','AUD','CAD','SGD','NZD','TWD','KRW','DKK','TRL','MYR','THB','INR','PHP','CHF','SEK','ILS','ZAR','RUB','NOK','AED', 'CNY', 'CZK', 'PLN'])) {
            return [
                'status' => '5',
        ...
        //$data = ["email" => 'user20006@gmail.com',"password"=>"qwerty12345"];
        ...
        if ( isset($result['statusCode']) && $result['statusCode'] == 200 && isset($result['data']['user']['token']) && $result['data']['user']['token'] != null) {
            ...
            $request_url = 'https://payments.cardpayz.com/api/payments';
            $request_header = [
