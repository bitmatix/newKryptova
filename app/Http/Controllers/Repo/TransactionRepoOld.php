...
class TransactionRepoOld extends Controller
{
    ...
    */// ==============================================
    public function store($input)
    {
        ...
        if (!in_array($input['payment_gateway_id'], $arrPaymentGateway)){
            ...
            // echo "<pre>";print_r($mid_limit_response);exit();
            
            // echo "<pre>";print_r($blocked_country_response);
            $user = $this->user->findData($input['user_id']);
            // if($blocked_country_response != false){
            //     if($blocked_country_response["status"] == 5){

            //     }
            // }
            if ($mid_limit_response != false || $blocked_country_response != false) {
                if($mid_limit_response["status"] == 5 || $blocked_country_response["status"] == 5 && !empty($user->multiple_mid) && $input["card_type"] != '3'){
                ...
                }else if($mid_limit_response["status"] == 5 && !empty($user->multiple_mid_master) && $input["card_type"] == '3'){
    ...
    */// ==============================================
    ...
    public function getAnotherMIDMaster($input, $check_assign_mid){
        ...
        $mainMID = '';
        $multiple_mid = [];
        //echo $user->multiple_mid_master;exit();
