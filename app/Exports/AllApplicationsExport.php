...
class AllApplicationsExport implements FromCollection, WithHeadings
{
    protected $id;
    ...
    public function collection()
    {
        ...
        if (!empty($slave_connection)) {
            ...
            _WriteLogsInFile($getDatabaseName . " connection from admin application", 'slave_connection');
        }
        ...
        if (isset($input['status']) && $input['status'] != '') {
            $data = $data->where('applications.status', $input['status']);
        }

        $data = $data->where('users.is_white_label', '0')->get();
