...
<html lang="en">
	<!--begin::Head-->
	<head>
		<title>{{ config('app.name') }} | Agent OTP</title>
		<meta charset="utf-8">
	    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	    <meta name="description" content="{{ config('app.name') }} Agent OTP">
		...
		<div class="d-flex flex-column flex-root">
			...
			<div class="d-flex flex-column flex-column-fluid bgi-position-y-bottom position-x-center bgi-no-repeat bgi-size-contain bgi-attachment-fixed" style="background-image: url('{{ storage_asset('/theme/assets/media/illustrations/sigma-1/14-dark.png') }}');">
				...
				<div class="d-flex flex-center flex-column flex-column-fluid p-10 pb-lg-20">
					<!--begin::Logo-->
					<a href="{{ route('rp/login') }}" class="mb-12">
					...
					<div class="w-lg-600px bg-body rounded shadow-sm p-10 p-lg-15 mx-auto">
						<!--begin::Form-->
						<form action="{{ route('rp.kryptova-otp-store') }}" id="kt_sing_in_two_steps_form" method="post" class="form w-100" novalidate="novalidate">
                        {!! csrf_field() !!}
							<!--begin::Heading-->
						...
						<div class="login-otp-main">
                            <a href="{{ route('rp.resend-otp') }}" class="btn btn-lg btn-danger w-100 mb-5 disabled" disabled id="resendOTP">Resend OTP <span id="countdown" >60s</span></a>
                        </div>
					</div>
					<!--end::Wrapper-->
				</div>
	        ...
	        var timeLeft = 60;
	                ...
	                function countdown() {
	                    ...
	                    if (timeLeft == 0) {
	                        ...
	                        document.getElementById("resendOTP").setAttribute("href", "{{ route('rp.resend-otp') }}");
	                        elem.innerHTML = '';
